...
if ! is_positive_integer "$BACKUP_MAX_AGE_HOURS"; then fail "MC_BACKUP_MAX_AGE_HOURS is invalid"; BACKUP_MAX_AGE_HOURS=0; fi
if ! is_positive_integer "$DISK_MAX_USED_PERCENT" || [ "$DISK_MAX_USED_PERCENT" -gt 100 ]; then fail "MC_DISK_MAX_USED_PERCENT is invalid"; DISK_MAX_USED_PERCENT=1; fi
if ! is_positive_integer "$STATUS_TIMEOUT_SECONDS"; then fail "MC_STATUS_TIMEOUT_SECONDS is invalid"; STATUS_TIMEOUT_SECONDS=5; fi
if ! is_valid_port "$SERVER_PORT"; then fail "MC_SERVER_PORT is invalid"; SERVER_PORT=19132; fi
...
if is_container_running && server_status >/dev/null 2>&1; then pass "Bedrock server responds to UDP status"; else fail "Bedrock server did not respond to UDP status"; fi
if docker port "$CONTAINER_NAME" "$SERVER_PORT/udp" 2>/dev/null | grep -q .; then pass "Bedrock UDP port is published"; else fail "Bedrock UDP port is not published"; fi
...
SERVICE_STARTED="$(last_service_started mc-backup.service)"
if [ -n "$SERVICE_STARTED" ] && [ "$SERVICE_STARTED" != "n/a" ] && [ "$SERVICE_RESULT" = "success" ] && [ "$SERVICE_STATUS" = "0" ]; then pass "Last backup service run succeeded"; else fail "Last backup service run failed or has not run"; fi
